...
def common_timeout():
    print "Oops Timeout!!!"
    os.system("(sleep 1; killall `basename " + sys.argv[0] + "`)")
...
def IsDaemonExist(daemon):
    command = "ps aux|grep %s" %(daemon)
    ret = commands.getstatusoutput(command)
    output = ret[1].split('\n')
    for item in output[:]:
        if item.find('grep %s' %(daemon)) != -1:
            output.remove(item)
    if ret[0] == 0 and len(output) > 0:
        print "Daemon is %s" %(output)
        return True
    else:
        print "daemon %s is not lauched" %(daemon)
        return False
    
def OneDaemonExist(daemon):
    command = "ps aux|grep %s" %(daemon)
    ret = commands.getstatusoutput(command)
    output = ret[1].split('\n')
    for item in output[:]:
...
class Manager:
    ...
    def RequestScan(self, type):
        ...
        bus = dbus.SystemBus()
        path = "/net/connman/technology/" + type
...
#                path = "/net/connman/technology/" +  tech
#                technology = manager.GetSubObject(path, 'Technology')
#                print 'Enabling %s...' % tech
#                try:
#                    technology.SetProperty("Powered", True)
...
#        return None
    def GetServiceByName(self, Name):
        ...
        for path, props in services:
            service = dbus.Interface(self.bus.get_object('net.connman'
                                   , path), 'net.connman.Service'
                                   )
            properties = service.GetProperties()
...
class Service:
    ...
    def BroadcastPing(self, size=0):
        ...
        try:
            if self.Ping(64, self.GetGateway()):
                return True
            if self.Ping(64, self.GetNameserver()):
                return True
        except dbus.DBusException, e:
            pass
	# ping otc-qa, an ugly solution :-(
        if self.Ping(64, cm_ping_server_ip):
            return True
        # Unfortuanately sometime test environment does not set
        # ping response server, and even filter icmp.
        # Since this function just test IP address work
        # So just check some properties are set by dhcp
        try:
            if (self.GetGateway() != None):
                return True
            if (self.GetNameserver() != None):
                return True
#            if (self.GetDomains() != None):
#                return True
        except dbus.DBusException, e:
            pass
    ...
    def GetIP(self):
        ...
        if self.properties == None:
            return None
        if 'IPv4' not in self.properties.keys():
            print 'No IPv4 properties'
            return None
    ...
    def GetNameserver(self):
        ...
        if self.properties == None:
            return None
        if "Nameservers" not in self.properties.keys():
            print 'Cannot get Nameservers for this device'
            return None
        return self.properties['Nameservers'][0]

    def GetGateway(self):
        self.properties = self.GetProperties()
        if self.properties == None:
            return None
        if "Gateway" not in self.properties['IPv4'].keys():
            print 'Cannot get Gateway for this device'
            return None
        return self.properties['IPv4']['Gateway']
...
class Device:
    ...
    def __init__(self, name, nth, enable = True):
        self.device = None
        self.intf = None
        self.tech = None
#        for path in manager.properties['Technologies']:
    ...
    def GetService(self, servicename=None):
        ...
        #                           , path), 'net.connman.Service'
        #                           )
#        self.manager.RequestScan(self.type)
...
#                    continue
    ...
    def IsPoweredOff(self):
        ...
        return properties['Powered'] == False
    ...
    def HasIP(self):
        ...
        return service != None and service.HasIP()
    ...
    def IsSCPWork(self):
        print 'scp to server'
        ret = commands.getstatusoutput(cm_scp_download)
        if ret[0] != 0:
            print 'scp failed'
            return False
        print 'scp success'
        print 'scp from server'
        ret = commands.getstatusoutput(cm_scp_upload)
        if ret[0] != 0:
            print 'scp failed'
            return False
        print 'scp success'
        commands.getstatusoutput(cm_ssh_rm)
        commands.getstatusoutput(cm_rm)
        return True
...
#        device_path = manager.ConnectService(ssid)
        ...
        if service == None:
            services = self.manager.GetServices()
            for path, props in services:
                service_ = dbus.Interface(self.manager.bus.get_object('net.connman'
                                         , path), 'net.connman.Service')
                print service_
                properties_ = service_.GetProperties()
                
                if properties_["Favorite"] == True and properties_["Type"] == 'wifi':
                    print "remove favoirte service: %s" % service_
                    service_.Remove()
        
        scan_times = 0
        while scan_times < 3 and service == None:
            time.sleep(60)
            self.manager.RequestScan('wifi')
            time.sleep(10)
            service = self.GetService()
...
class WiFiGuestDevice(WiFiDevice):
    ...
    def __init__(self, ssid=cm_open_ap_essid):
        WiFiDevice.__init__(self)
        service = Device.GetService(self, self.ssid)
        print 'Now connect...'
        self.Enable()
#        service.Connect()
        WiFiDevice.Connect(self, cm_open_ap_essid)
...
try:
    #change the time out from 180 to 360
    t = Timer(300, common_timeout)
    t.daemon = True
    t.start()
except:
    print "oops"
