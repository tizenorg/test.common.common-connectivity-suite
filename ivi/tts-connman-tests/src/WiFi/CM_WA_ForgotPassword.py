...
if not svc == None:
    try:
        if svc.GetProperty("Favorite"):
            svc.Remove()
    except dbus.DBusException, error:
        pass

os.chdir(os.path.dirname(sys.argv[0]))
proc = subprocess.Popen("./simple-agent Name=%s Passphrase=%s" % (cm_wpa2_ap["name"], cm_wpa2_ap["password"] + '1'), shell=True)
os.chdir(os.getcwd())
dev = WiFiDevice()
try:
...
svc = dev.GetService()
if svc.GetProperty("State") == "failure":
    print "The state is %s" % svc.GetProperty("State")
    EXIT(True)
else:
    print "The state is %s" % svc.GetProperty("State")
    EXIT(False)
