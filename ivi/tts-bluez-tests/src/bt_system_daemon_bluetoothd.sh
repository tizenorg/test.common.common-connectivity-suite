...
bt_daemon_pid=`ps aux | grep bluetoothd | grep -v grep | awk '{print $2}' | head -n 1`

if [ -z "$bt_daemon_pid" ]; then
    echo "bluetoothd daemon isn't started, this is a crital failure"
    echo "[FAIL] Check if bluetoothd daemon started: Failed"
    exit 1
else
    echo "Check if bluetoothd daemon started: Successfully"
    echo "[PASS] bluetoothd daemon has been started, the pid is: $bt_daemon_pid"
    exit 0
fi
