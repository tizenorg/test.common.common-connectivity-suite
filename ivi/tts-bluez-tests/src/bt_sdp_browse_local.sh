...
# Local device should support SDP Server.
service_record=`sdptool browse local | grep "Service Name"`
line=`echo $service_record | wc -l`
if [ 0 -eq $line ]; then
    echo "[FAIL] No SDP Server support"
    ret=1
else
    echo "[PASS] Device support SDP Server, local service list is:"
    echo "$service_record"
fi
